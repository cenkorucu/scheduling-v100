...
export const generateSchedule = (residents, rotations) => {
  ...
  // Step 5: Placeholder (Empty for Now)
  // TODO: Add logic here if needed in the future
  ...
  // Step 9: Limit Mandatory Rotations to Maximum Allowed (Run 20 Times)
  ...
  // Step 10: Resolve Overfilled Mandatory Rotations per Block (Up to 20 Passes)
  ...
  // Step 11: Enforce Maximum Limits for Mandatory Rotations per Resident (Run 26 Cycles)
  for (let cycle = 0; cycle < 26; cycle++) {
    mandatoryRotations.forEach((rotation) => {
      const maxAllowed = rotation.max || Infinity;
      residents.forEach((resident) => {
        const currentCount = rotationCounts[resident.name][rotation.name] || 0;
  ...
  // Step 12: Fill Underfilled Mandatory Rotations with Lowest Count Residents (Run 26 Cycles)
  for (let cycle = 0; cycle < 26; cycle++) {
    for (let block = 0; block < 26; block++) {
      mandatoryRotations.forEach((rotation) => {
        const required = rotation.requiredPerBlock || 1;
  ...
  // Step 14: Balance Rotation Counts Across Residents
  rotations.forEach((rotation) => {
    residents.forEach((residentA, indexA) => {
      for (let indexB = indexA + 1; indexB < residents.length; indexB++) {
        const residentB = residents[indexB];
        const countA = rotationCounts[residentA.name][rotation.name] || 0;
        const countB = rotationCounts[residentB.name][rotation.name] || 0;
        const diff = Math.abs(countA - countB);

        if (diff >= 2) {
          const higherResident = countA > countB ? residentA : residentB;
          const lowerResident = countA > countB ? residentB : residentA;
          const higherCount = Math.max(countA, countB);
          const lowerCount = Math.min(countA, countB);
          const targetDiff = Math.floor(diff / 2); // Number of swaps needed to reduce diff to 0 or 1

          let swapsNeeded = targetDiff;
          for (let block = 0; block < 26 && swapsNeeded > 0; block++) {
            const higherAssignment = schedule[higherResident.name][block];
            const lowerAssignment = schedule[lowerResident.name][block];

            // Check if higher resident has the rotation and lower has a swappable slot
            if (
              higherAssignment === rotation.name &&
              lowerAssignment !== 'Ambulatory' &&
              lowerAssignment !== 'Vacation' &&
              (!rotation.mandatory || residents.filter((r) => schedule[r.name][block] === rotation.name).length > (rotation.requiredPerBlock || 1))
            ) {
              // Perform the swap
              schedule[higherResident.name][block] = lowerAssignment;
              schedule[lowerResident.name][block] = rotation.name;

              // Update counts
              rotationCounts[higherResident.name][rotation.name]--;
              rotationCounts[lowerResident.name][rotation.name] = (rotationCounts[lowerResident.name][rotation.name] || 0) + 1;
              if (lowerAssignment !== '-') {
                rotationCounts[higherResident.name][lowerAssignment] = (rotationCounts[higherResident.name][lowerAssignment] || 0) + 1;
                rotationCounts[lowerResident.name][lowerAssignment]--;
              }
  ...
  // Step 13: Assign Non-Mandatory Rotations with Exact Constraints
